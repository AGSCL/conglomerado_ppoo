...
message("Descartando valores negativos en sil width")
chisq.test(
with(subset(ing_dt_ing_calendar_quarter_t_desde_primera_adm_dedup_wide2_cens, !rn %in% sil_neg_pam_om_clus4_q), table(pueblo_originario_01 , clus_pam_om4))
)
```
...
chisq_cramerv(tab_clus_macrozona_pam_om4_q[,-1])
# $chisq_statistic
# [1] "125.88"
# 
# $chisq_df
...
dplyr::mutate(macrozona = dplyr::case_when(
  ...
  codigo_region %in% c(11, 12) ~ "Macrozona Austral",
  TRUE ~ "RM"  # En caso de que algún código no esté especificado
)) , table(macrozona, clus_pam_om4))
)

tab_clus_macrozona_pam_om4_q%>% 
  janitor::adorn_percentages("col") %>% 
  janitor::adorn_rounding(digits = 2) %>% 
    dplyr::mutate(across(-macrozona, ~ sprintf("%d (%.0f%%)", tab_clus_macrozona_pam_om4_q[[cur_column()]], . * 100))) |> 
...
chisq.posthoc.test(tab_clus_macrozona_pam_om4_q[-1]) %>% 
  dplyr::mutate_at(3:ncol(.), ~round(as.numeric(gsub("\\*","",.)),3)) %>% 
  #knitr::kable("html", caption="Comparación post-hoc, conglomerado-región")%>%
  group_by(Dimension) %>%
  summarise(across(2:(ncol(.)-1),
...
#| results: "hold"
#| 
pairwise_chisq_gof_test(tab_clus_macrozona_pam_om4_q[-1], p.adjust.method="holm")|> 
...
tab_clus_sexo_pam_om4_q%>% 
  janitor::adorn_percentages("col") %>% 
  janitor::adorn_rounding(digits = 2) %>% 
  dplyr::mutate(across(-glosa_sexo, ~sprintf("%d (%.0f%%)", tab_clus_sexo_pam_om4_q[[cur_column()]],.*100)))|>
...
ggsave("_figs/edad_minima_por_cluster_pam_om4_q.png", dpi=600)
```
...
tab_clus_prev_pam_om4_q%>% 
  ...
  dplyr::mutate(across(-prev_benef_rec_post, ~sprintf("%d (%.0f%%)", tab_clus_prev_pam_om4_q[[cur_column()]],.*100))) |> 
  #dplyr::mutate_at(2:ncol(.),~scales::percent(.)) %>% 
...
ggplot(aes(x = Variable, y = Grupo, fill = Asociación)) +
  ...
  coord_flip()
ggsave("_figs/asociaciones_pam_om4_q.png", width=8.8*.8, height=5*.8, dpi=1000)
