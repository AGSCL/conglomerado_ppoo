...
El conglomerado **5939** conserva el **88%** de las observaciones originales. Del **12% restante**, un **9%** proviene del conglomerado original **6035** y se dirige al **6036**, el cual está **100%** compuesto por este conglomerado. Por otro lado, un **3%** se transfiere al conglomerado **5935**, que está conformado en un **93%** por observaciones provenientes del conglomerado original. Finalmente, el **2%** restante se incorpora al conglomerado **5710**, que está compuesto en un **70%** por este mismo grupo.

Otro conglomerado que actúa como donante es el **5939**, del cual se desprende un **11%** de sus observaciones. De este porcentaje, un **9%** se transfiere al conglomerado **5710**, donde representa el **23%** de este último, y un **2%** se dirige al conglomerado **5935**, representando el **6%** de dicho grupo.
...
seq_plot_pam_om7_q <- ggseqiplot(States_Wide.seq_quarter_t_prim_adm_cens, 
                                 ...
                                 facet_ncol=3, facet_nrow=3, sortv=sil_pam_om_clus7_q) +
...
ggsave(filename="_figs/clusters_pam_om7_q_mod.png", seq_plot_pam_om7_q,  width = 11, height = 5.5, dpi=1000)
```
...
seq_plot2_pam_om7_q
ggsave("_figs/clusterspam_om72_q_mod.png",seq_plot2_pam_om7_q, width = 8.5, height = 5.5, dpi=1000)
...
chisq_cramerv(tab_clus_macrozona_pam_om7_q[,-1])
# $chisq_statistic
# [1] "143.52"
# 
# $chisq_df
...
dplyr::mutate(macrozona = dplyr::case_when(
  ...
  codigo_region %in% c(11, 12) ~ "Macrozona Austral",
  TRUE ~ "RM"  # En caso de que algún código no esté especificado
)) , table(macrozona, clus_pam_om7))
)

tab_clus_macrozona_pam_om7_q %>%
  janitor::adorn_percentages("col") %>%
  janitor::adorn_rounding(digits = 2) %>%
  dplyr::mutate(across(-macrozona, ~ sprintf("%d (%.0f%%)", tab_clus_macrozona_pam_om7_q[[cur_column()]], . * 100))) |> 
...
chisq.posthoc.test(tab_clus_macrozona_pam_om7_q[-1]) %>% 
  dplyr::mutate_at(3:ncol(.), ~round(as.numeric(gsub("\\*","",.)),3)) %>% 
  #knitr::kable("html", caption="Comparación post-hoc, conglomerado-región")%>%
  group_by(Dimension) %>%
  summarise(across(2:(ncol(.)-1),
                   ~ paste0(first(sprintf("%1.2f",.)), " (p=", last(sprintf("%1.3f",.)), ")"))) %>%
  dplyr::mutate_at(2:length(names(.)), ~gsub("p\\=0.000)","p<0.001)",.)) %>% 
  dplyr::mutate(Dimension= tab_clus_macrozona_pam_om7_q[1]) %>% 
...
#| results: "hold"
#| 
pairwise_chisq_gof_test(tab_clus_macrozona_pam_om7_q[-1], p.adjust.method="holm")|> 
...
tab_clus_sexo_pam_om7_q%>% 
  janitor::adorn_percentages("col") %>% 
  janitor::adorn_rounding(digits = 2) %>% 
  dplyr::mutate(across(-glosa_sexo, ~sprintf("%d (%.0f%%)", tab_clus_sexo_pam_om7_q[[cur_column()]],.*100)))|> 
...
tab_clus_prev_pam_om7_q%>% 
  ...
  dplyr::mutate(across(-prev_benef_rec_post, ~sprintf("%d (%.0f%%)", tab_clus_prev_pam_om7_q[[cur_column()]],.*100)))|>
  #dplyr::mutate_at(2:ncol(.),~scales::percent(.)) %>% 
...
tab_clus_compl_pam_om7_q%>% 
  janitor::adorn_percentages("col") %>% 
  janitor::adorn_rounding(digits = 2) %>% 
  dplyr::mutate(across(-nivel_de_complejidad, ~sprintf("%d (%.0f%%)", tab_clus_compl_pam_om7_q[[cur_column()]],.*100)))|>
