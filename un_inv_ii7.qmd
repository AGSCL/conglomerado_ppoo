...
load(paste0(folder_path,"avance250117_2.RData"))
```
...
ing_dt_ing_calendar_month_t_desde_primera_adm_dedup_wide2_cens$clus_pam_om2 <-
  factor(pamRange_month_lcs$clustering$cluster2,levels=rev(attr( sort(table(pamRange_month_lcs$clustering$cluster2)), "name")),
         labels= c("6035, Un trimestre, TSM(2)", 
...
sil_pam_om_clus2_m_nostd<-
  silhouette(as.integer(pamRange_month_om$clustering$cluster2), as.dist(dist_quarter_om))
...
km_data <- data.frame(
  ...
  upper = km_fit$upper,
  lower = km_fit$lower,
  strata = rep(c("6035, Un trimestre, TSM(2)","6025, Un trimestre, TUS(1)"), km_fit$strata)
)
...
Se constata una asociación entre la pertenencia a un conglomerado y mortalidad, siendo 6025, Un trimestre, TUS(1)	mayor sus chances de mortalidad.
